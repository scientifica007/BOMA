...
def main() -> int:
    ...
    if generation != "BOMA-AUTONOMY-003":
        print("Refusing START: experiment generation is not BOMA-AUTONOMY-003", file=sys.stderr)
        return 5
    ...
    window_id = f"MW-{now_dt.strftime('%Y%m%dT%H%M%SZ')}-{hours}H-G3"
    ...
    print(f"BOMA AUTONOMY GENERATION 003 STARTED at {now} on {head}; observation deadline {deadline}")
    return 0
