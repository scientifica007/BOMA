...
def main() -> int:
    ...
    if generation != "BOMA-AUTONOMY-002":
        print("Refusing START: experiment generation is not BOMA-AUTONOMY-002", file=sys.stderr)
        return 5
    ...
    window_id = f"MW-{now_dt.strftime('%Y%m%dT%H%M%SZ')}-{hours}H-G2"
    ...
    print(f"BOMA AUTONOMY GENERATION 002 STARTED at {now} on {head}; observation deadline {deadline}")
    return 0
